...
export async function generateMetadata({
  ...
  try {
    const profile = await getBloggerProfile(username);
    if (!profile) {
      return {
        title: 'Blogger Not Found - Goodsongs',
        description: 'The requested music blogger profile could not be found.',
      };
    }
    const reviewsCount = profile.reviews_pagination?.total_count ?? profile.reviews?.length ?? profile.reviews_count ?? 0;
    return {
      title: `@${profile.username} - Music Blogger - Goodsongs`,
...
async function getBloggerProfile(username: string): Promise<PaginatedUserProfile | null> {
  ...
  const response = await fetch(`${baseUrl}/api/blogs/${username}?page=1&per_page=20`, {
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error('Failed to fetch blogger profile');
  }

  return response.json();
}

export default async function BloggerProfilePage({
  params,
}: {
  params: Promise<{ username: string }>;
}) {
  ...
  try {
    profile = await getBloggerProfile(username);
  } catch (error) {
    return (
      <Container>
