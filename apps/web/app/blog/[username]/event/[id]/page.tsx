...
import { getEventById, getPublicProfile } from '@/lib/site-builder/api';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: `${event.name} - @${username} - Goodsongs`,
...
export default async function BloggerEventPage({ params }: PageProps) {
  const { username, id } = await params;
  const eventId = parseInt(id, 10);
  ...
  try {
    ...
    // Profile not found, continue without theme
  }
  ...
  const profileBasePath = `/blog/${username}`;
  const profileName = publicProfile?.data?.user?.display_name || username;
  const profileImageUrl = publicProfile?.data?.user?.profile_image_url;
        ...
        <Stack gap="xl">
          ...
          <Stack gap="md">
            ...
            <Group gap="md">
              <Group gap="xs">
                <Avatar
                  src={fixImageUrl(profileImageUrl)}
                  alt={profileName}
                  size="sm"
                  radius="xl"
                />
