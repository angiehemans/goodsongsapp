...
export default function ProEventsPage() {
  const { user, isLoading, isBlogger } = useAuth();
  const { band } = useBand();
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      if (isBlogger) {
        fetchedEvents = await apiClient.getUserEvents(user.id);
      } else if (band?.slug) {
        fetchedEvents = await apiClient.getBandEvents(band.slug);
      } else {
        fetchedEvents = [];
      }
  ...
  }, [user, isBlogger, band?.slug]);

  useEffect(() => {
    if (user && (isBlogger || band)) {
      fetchEvents();
    }
  }, [user, isBlogger, band, fetchEvents]);
  ...
  return (
    <Container size="md" py="xl" px="md">
      ...
      <EventForm
        bandSlug={!isBlogger && band ? band.slug : undefined}
        event={editingEvent}
