...
function BloggerDashboard() {
  ...
  // Defer chart rendering until after mount so the container has dimensions
      ...
      <Stack gap="lg" maw={1200}>
            ...
            <Paper p="md" radius="md" withBorder h="100%">
              <Group justify="space-between" mb="md">
                <div>
                  ...
                  <Text size="xs" c="dimmed">Last 60 days</Text>
                </div>
                  ...
                  <Stack gap="xs" mt="md">
                    {trafficSourcesData.map((source) => (
                      <Group key={source.name} justify="space-between">
                        ...
                        <Text size="sm" fw={500}>{source.value.toFixed(1)}%</Text>
                      </Group>
                    ))}
                  </Stack>
                </>
                    ...
                    <Table.Tbody>
                      {dashboardData.recent_posts.map((post) => (
                        <Table.Tr key={post.id}>
                          <Table.Td>
                            <Text size="sm" lineClamp={1} maw={250}>{post.title}</Text>
                          </Table.Td>
                          <Table.Td>
                            <Badge variant="light" color={getStatusColor(post.status)} size="sm" tt="capitalize">
                              {post.status}
                            </Badge>
                          </Table.Td>
                          <Table.Td ta="right">
                    ...
                    return (
                      <Group key={notification.id} gap="sm" wrap="nowrap">
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <Text size="sm" fw={500} lineClamp={1}>{actorName}</Text>
        ...
        <Grid>
          <Grid.Col span={{ base: 12, md: 5 }}>
            <Paper p="md" radius="md" withBorder style={{ height: '-webkit-fill-available' }}>
              <Text fw={600} mb="md">Follower Growth</Text>
              {loading || !chartsReady ? (
                <Skeleton height={200} />
          ...
          <Grid.Col span={{ base: 12, md: 7 }}>
            <Paper p="md" radius="md" withBorder style={{ height: '-webkit-fill-available' }}>
              <Text fw={600} mb="md">Top Performing Posts</Text>
              {loading ? (
                <Stack gap="md">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Skeleton key={i} height={40} />
                    ...
                    <div key={post.id}>
                      <Group justify="space-between" mb={4}>
                        <Group gap="xs">
                          ...
                          <Text size="sm" lineClamp={1}>{post.title}</Text>
                        </Group>
                        <Text size="sm" fw={500}>{post.views.toLocaleString()} views</Text>
                      </Group>
                      <Progress
        ...
        <Card p="lg" radius="md" withBorder bg="var(--mantine-color-grape-light)">
          <Group justify="space-between" wrap="nowrap">
            <div>
              <Text fw={600} mb={4}>Ready to grow your audience?</Text>
              <Text size="sm" c="dimmed">
...
function BandDashboard() {
  const { user } = useAuth();
  const { band, isLoading: bandLoading } = useBand();
  const [dashboardData, setDashboardData] = useState<BlogDashboardResponse | null>(null);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const dashboardPromise = apiClient.getBlogDashboard();
        const eventsPromise = band ? apiClient.getBandEvents(band.slug) : Promise.resolve([]);
        const [dashboard, events] = await Promise.all([dashboardPromise, eventsPromise]);
  ...
  const upcomingEvents = useMemo(() => {
    const now = new Date();
    return bandEvents.filter((e) => new Date(e.event_date) >= now);
  }, [bandEvents]);
  ...
  const maxEngagement = dashboardData?.top_performing_posts[0]?.engagement_score || 1;
  const bandReviews = band?.reviews || [];
  ...
  const isLoading = loading || bandLoading;
      ...
      <Stack gap="lg" maw={1200}>
        ...
        <Group justify="space-between" align="flex-start">
          <div>
            ...
            <Text size="sm" c="dimmed">
              Welcome back, {band?.name || user?.display_name || user?.username}
            </Text>
          </div>
          <Button
            component={Link}
        ...
        <SimpleGrid cols={{ base: 1, xs: 2, md: 4 }}>
          ...
          <StatCard
            ...
            value={dashboardData?.totals.recommendations || bandReviews.length}
            icon={IconMusic}
            color="blue"
            loading={isLoading}
          />
        ...
        <Paper p="md" radius="md" withBorder>
          <Group justify="space-between" mb="md">
            ...
            <Button component={Link} href="/user/pro/events" variant="subtle" color="grape" size="xs">
              Manage Events
            </Button>
          </Group>
          {isLoading ? (
            ...
            <Center py="lg">
              <Stack align="center">
                ...
                <Button component={Link} href="/user/pro/events" variant="light" leftSection={<IconPlus size={16} />}>
                  Create Event
                </Button>
              </Stack>
            </Center>
        ...
        <Card p="lg" radius="md" withBorder bg="var(--mantine-color-grape-light)">
          <Group justify="space-between" wrap="nowrap">
            <div>
              <Text fw={600} mb={4}>Ready to grow your audience?</Text>
              <Text size="sm" c="dimmed">
                Customize your band page and share it with fans to get more recommendations.
              </Text>
            </div>
            <Button
              component={Link}
